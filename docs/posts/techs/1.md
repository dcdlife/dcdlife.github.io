...
HTML 标签分类主要有两种：一般标签（开+闭）和自闭合标签。一般标签可以往里面添加内容，比如`<div>Hello</div>`，自闭合标签不可以添加内容，比如：`<hr>、<br>`。所以如果想插入文字或别的标签，它一定不是自闭合标签。

另一个非常重要的概念，HTML 标签的表现形式，一般分类两种：块元素和行内元素。块元素独占一行，行内元素则独占一块，这一块根据它的内容来确定。块元素常见的有 div、h1-h6、p、hr、ul/ol/li 等，行内元素常见的有：span、strong、em、a

HTML 标签用途分类主要有： 文档类型标签、基本标签、文本展示类标签、列表标签、表格标签、图片标签、超链接标签、表单标签、框架标签。
...
包含 3 个，分别是 html、head、body。html 代表一颗完整的树，head 可以理解为看不见的树根，body 则可以理解为地平面以上我们能看到的树的所有外在。因此 head 里面放一些我们看不见的元信息，比如：页面标题、脚本引用、样式引用等等。body 则放置一切用于显示的标签。

对于基本标签我们重点关注 head 即可，head 里面可以放的元素有 6 个：title、meta、link、script、style、base。

这里重点关注 meta 和 link。
meta 用于定于页面的特殊信息，也是用来告诉爬虫一些信息的地方。meta 主要有 2 个重要的属性：name、http-equiv，且需要搭配 content 一起使用。
...
link 可以用于：网页的 icon、引入外部的样式表。

网页的内容主要由文字、图片、超链接、音频和视频组成。

无论内容是什么形式，我们都会用到一个万能的元素将内容进行分块，划分出一块块区域，它就是 div，你甚至可以只使用 div+css 来编写网页，但为了语义化一般都会搭配对应的语义化元素。

文本展示类

h1-h6（标题标签）、p（段落标签）、br（换行标签）、重要（文本标签）、hr（水平线标签）、特殊符号。
...
3. sup（上标标签）
4. sub（下标标签）
5. s（中划线标签）一般通过 css，也不怎么用 s
6. u（下划线标签）一般通过 css，也不怎么用 u
7. big（大字号）一般通过 css，也不怎么用 big
8. small(小字号) 一般通过 css，也不怎么用 small，所以，常用的文本标签也就：strong、em、sup、sub。

特殊符号
由于 html 的解析机制会导致一些正常的内容无法显示，比如空格，大于号和小于号等，所以需要用特殊的内容来编写，这样 h 浏览器才能解析成我们想要展示的内容。
...
2. （用的更多）无序列表对应：ul（可指定 type 属性更改前面的数字样式）、li。
...
高清用 jpg，一般图片 png，动图用 gif。
矢量图：用数学描述的方式来表示图片的内容。

超链接标签
超链接可以链接到的地方有 2 个：网页内(#id)和网页外(url)，可以用 a 标签包括文字或图片，来实现点击后的跳转，另外点击后的打开方式通过 target 属性设置，支持 \_self/\_blank/\_parent/\_top。
<a href="" >点我跳转</a>

表单标签
用于提交信息到服务端，根据标签名分类有 5 种：form、input、textarea、select 和 option。根据外观表单内的元素可以分为 8 种：单行文本框、多行文本框、密码文本框、单选框、多选框、按钮、文件上传、下拉列表。

表单包裹在 form 标签内，它有 5 个属性：name、method、action、target、enctype(用于表单数据的编码方式，一般不设置，除非用到文件上传)。
